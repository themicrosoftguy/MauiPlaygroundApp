...
using MauiPlaygroundApp.Models;
...
namespace MauiPlaygroundApp.ViewModels
{
    public class ReadSMSMessagesViewModel : BaseViewModel
    {
        ...
        //public ObservableCollection<SMSMessageModel> DebitTransactions { get; } = new ObservableCollection<SMSMessageModel>();

        //public ObservableCollection<SMSMessageModel> CreditTransactions { get; } = new ObservableCollection<SMSMessageModel>();
        ...
        {
            var keywords = new[] { "Acct XX035 is credited with", "Acct XX035 credited", "successfully debited", "Acct XX0035 debited" }; // Replace with actual keywords
        ...
        {
            var debitKeywords = new[] { "successfully debited", "Acct XX0035 debited" }; // Replace with actual debit keywords
        ...
        {
            var creditKeywords = new[] { "Acct XX035 is credited with", "Acct XX035 credited" }; // Replace with actual credit keywords
        ...
        private bool ContainsKeywordsAndExtractAmount(string message, string[] keywords, out double amount)
        {
            ...
            foreach (var keyword in keywords)
            {
                if (message.Contains(keyword, StringComparison.OrdinalIgnoreCase))
                {
                    var match = Regex.Match(message, @"Rs\s\d+(\.\d{1,2})?");
                    if (match.Success)
                    {
                        var amountString = match.Value.Replace("Rs", "").Trim();
